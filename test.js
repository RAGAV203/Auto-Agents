document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        // Start the process via fetch to get the task ID
        fetch('/start_process', {
            method: 'POST',
        ...
        .then(response => {
            if (response.status === 202) {
                return response.text(); // Get the Task ID from response
            } else {
                throw new Error("Failed to start the process.");
            }
        })
        .then(taskId => {
            console.log(`Task ID received: ${taskId.trim()}`); // Log the Task ID
            // Once we have the task ID, we fetch the result stream
            streamTaskResult(taskId.trim());
        })
        .catch(error => {
    ...
    function streamTaskResult(taskId) {
